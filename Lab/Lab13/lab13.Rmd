...
<!---
# Example
How does this approximate collinearity affect our coefficient estimates?
```{r}
lm1 = lm(Infection.risk ~ Beds + Patients, senic)
round(summary(lm1)$coefficients,3)
```
`Beds` has a negative estimated coefficient and none of the estimates are significantly different from 0.

# Example
If we fit a model with just `Beds`, the sample coefficient is positive and the statistically different from zero.
```{r}
