...
- Essentially HW 1-3 and notes 1-4 (ending Wednesday)
...
- Recall: A random variable can take on discrete (e.g. a die: {1, 2, 3, 4, 5, 6}) or continuous values (e.g. IQ following a normal distribution)

\vspace{-10mm}
- \textcolor{blue}{Suggestion: Review from STATS 250 the concepts of expected value and variance and the properties of common distrbutions such as the normal, t, and F.}

# In Lab Exercises (Part 1)

- Show $\sum_{i=1}^n(x_i - \bar{x})^2 = \sum_{i=1}^n(x_i^2) - (\bar{x})^2$.
- Show how **y** = $\mathbb{X}$**b**, with n observations, can be written as a sum.
...
Using the matrices from the lab exercise, calculate the matrices returned by following r command:
...
- Suppose we draw 20 sugar pumpkins at random. What is the probability that the average weight of the pumpkins will be less than 2.3 lbs.