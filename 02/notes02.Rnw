...
\answer{\vspace{25mm}}{This physics definition corresponds to a polar coordinate representation of our vectors in \mymath{\Rspace^2} and \mymath{\Rspace^3}. \vspace{5mm}}

{\myquestion}. 
How can I write a boldface \mymath{\vec{x}} in handwriting?
...
{\bf Worked example}. An ecologist measures the pH of ten Michigan lakes at two points in the summer. Set up vector notation to describe her data algebraically. Write a vector calculation, using summation notation, giving the average pH in each lake.
...
\answer{\vspace{20mm}}{Convenience. It is quite common to want to add a fixed amount to each data point, so R makes this easy.}
...
\item We write \m{\underset{n\times p}{\mat{A}}} to emphasize the dimension. Keeping track of matrix dimensions can be helpful!
\end{myitemize}
\end{frame}
...
\item 
We define matrix multiplication \m{\mat{A}\vec{x}=\vec{b}} to match this linear system. The matrix multiplication
\mydisplaymath{
\mymatrix{a}{n}{p}
\myvector{x}{p}
=
...
\item Matrices are good for using computers to solve large systems of linear equations.
