...
\item \m{\mat{A}+s\mat{1}} matches addition of a scalar and matrix in R.
\end{myitemize}
...
\begin{myitemize}
\item The main difference between matrix algebra and the usual numeric algebra is that matrix multiplication does not commute (\m{\mat{A}\,\mat{B} \neq \mat{B}\, \mat{A}}).
\item Many other properties for matrices are the same as for numbers.
\item Optional exercise: set up R examples to check the following.
\end{myitemize}

\mydisplaymath{
\begin{array}{rclrcl}
(\mat{A}\mat{B})\mat{C} &=& \mat{A}(\mat{B}\mat{C}) \hspace{10mm} &
...
\frametitle{More properties of matrices}
\begin{myitemize}
\item The inverse of a product is the product of the inverses, but with the order reversed.
\mydisplaymath{
\begin{array}{rclrcl}
