...
\begin{frame}[fragile]
\frametitle{The variance matrix of a vector random variable}

\vspace{-1.5mm}

\begin{myitemize}
\item Let \m{\vec{X}=(X_1,\dots,X_n)} be a vector random variable.
\item For any pair of elements, say \m{X_i} and \m{X_j}, we can compute the usual scalar covariance, \m{v_{ij}=\cov(X_i,X_j)}.
\item The variance matrix \m{\mat{V}=[v_{ij}]_{n\times n}} collects together all these covariances.
\mydisplaymath{
\mat{V}=\left[ 
\begin{array}{cccc}
...
\item The diagonal entries of \m{\mat{V}} are \m{v_{ii}=\cov(X_i,X_i) = \var(X_i)} for \m{i=1,\dots,n}. This lets us write the variance matrix as
\mydisplaymath{
\mat{V}
=
\left[ 
...
\cor(\vec{X})=\left[ 
\begin{array}{cccc}
1 & \cor(X_1,X_2) & \dots & \cor(X_1,X_n) 
\\
\cor(X_2,X_1) & 1 &  & \cor(X_2,X_n) 
\\
\vdots &  & \ddots & \vdots
\\
\cor(X_n,X_1) & \cor(X_n,X_2) & \dots & 1 
\end{array}
\right]
}
...
%\item Remarkably, the multivariate normal distribution depends 
\end{myitemize}
\end{frame}
