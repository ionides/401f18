...
\item 
If \m{X} and \m{Y} are bivariate random variables we need another parameter to describe their dependence. If \{X} is big, does \m{Y} tend to be big, or small, or does the value of \m{X} make no difference to the outcome of \m{Y}?
\item This parameter is called the \myemph{covariance}.
...
\item In R, the \code{mvtnorm} package lets us simulate the bivariate and multivariate normal distribution. It uses the vector and matrix form for the parameters.
...
@
\begin{myitemize}
\item We write \m{(X,Y)\sim MVN(\mu,\mat{V})}, where MVN abbreviates "multivariate normal".
\end{myitemize}
\myquestion. What are \m{\mu_X}, \m{\mu_Y}, \m{\var(X)}, \m{\var(Y)}, and \m{\cov(X,Y)} for this example?
