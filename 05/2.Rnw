...
\mydisplaymath{
\E[\vec{X}]=(\E[X_1],\E[X_2],\dots,\E[X_n])
}
...
\item This formula extends to any linear combination of \m{n} random variables. If \m{\vec{Z}=\mat{A}\vec{X}} then \m{\E[Z]=\mat{A}\E[X]}. In other words,
...
\item Our model \myref{\LMvi} asserts that the process which generated the response data \m{\vect{y}} was like drawing a random vector \m{\vect{Y}} constructed using a random measurement error model with known matrix \m{\mat{X}} for some fixed but unknown value of \m{\vect{\beta}}.
...
\WorkedExample.
Let \m{\vect{\hat\beta}= \big( \mat{X}^\transpose \mat{X} \big)^{-1}\, \mat{X}^\transpose \vect{Y}}. This is the probability model for the coefficient \m{\vect{b}= \big( \mat{X}^\transpose \mat{X} \big)^{-1}\, \mat{X}^\transpose \vect{y}} calculated using the data \m{\vec{y}}.
...
\myquestion. Use the formula \m{\var(\mat{A}\vec{Y})=\mat{A}\var(Y)\mat{A}^\transpose} to find \m{\var(\vec{\hat\beta})}. Hint: what should be our choice of \m{\mat{A}} so that \m{\vec{\hat\beta}= \mat{A}\vec{Y}}?
