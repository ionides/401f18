...
\item Since $\vec{b}$ is a noisy estimate of $\beta$, $\vec{e}$ is a noisy estimate of $\vec{\epsilon}$.
...
\answer{\vspace{10mm}}{The residuals are centered on zero by their construction, as long as there is an intercept in the model.}

\myquestion. How would you fit this linear model for the residuals in R?

\answer{\vspace{15mm}}{\code{
n <- length(L_detrended) \\
e <- resid(lm1)[2:n] \\
lag_e <- resid(lm1)[1:(n-1)]
lm(e~lag_e-1)
}}
...
\answer{\vspace{40mm}}{A cluster of x-values and an outlier in the x direction which is close to the linear regression line in the y direction.
...
\answer{\vspace{40mm}}{A cluster of x-values and an outlier in the x direction which is also an outlier from the linear regression line in the y direction.}
...
\answer{\vspace{40mm}}{An outlier in the middle of a simple linear regression  scatterplot.}
...
\item Taking the logarithm of non-negative data may help in this case.
