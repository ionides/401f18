...
\item Statistical properties of \m{\vect{Y}=\mat{X}\vect{\beta}+\vect{\epsilon}} are usually carried out using a probability model that \m{\epsilon_1,\dots,\epsilon_n \sim\iid \; \normal(0,\sigma)}.
\item independent identically distributed (iid) random variables have no pattern.
...
\answer{\vspace{5mm}}{Look at the axes. Careful reading of the code reveals (after plenty of head-scratching) that }
