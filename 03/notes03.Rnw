...
\m{\vec{x}_i=(x_{1i},x_{2i},\dots,x_{ni})} is the column vector of the \m{i}th predictor. 
\item The matrix form of the linear model, \m{\vec{y} = \mat{X}\, \vec{b} + \vec{e}}, can then be expanded to
\mydisplaymath{
\left[ \begin{array}{c}
y_1 \\
y_2 \\
