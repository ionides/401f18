...
\begin{myitemize}
\item We seek the \myemph{least squares} choice of \m{\vec{b}} that minimizes the \myemph{residual sum of squares}, \m{\sum_{i=1}^n e_i^2}.
...
\item
In general, \m{b_1,\dots,b_p} are called the \myemph{coefficients} of the linear model, and \m{\vec{b}} is the \myemph{coefficient vector}.
\item
...
{\bf This material will not be tested in quizzes and exams. It is presented to explain where the formula \myref{\LMiv} for \m{\vec{b}} comes from.}
...
%\myquestion
{\bf Worked example}. Apply the chain rule to differentiate one term in the sum. Check that
\mydisplaymath{
...
%\myquestion
{\bf Worked example}.
Since the derivative of a sum is the sum of the derivatives, check that
\mydisplaymath{
...
\mydisplaymath{
\vec{b}=\big[\mat{X}^\transpose\mat{X}\big]^{-1}\max{X}^\transpose\vec{y}
}
