...
Example:  For a die, \m{E=\{X\ge 4\}=\{4,5,6\}} is the event that the die lands on \m{4} or more.
...
\begin{enumerate}
\item Probabilities are non-negative numbers between 0 (impossible) and 1 (certain).
...
\myquestion. I roll a red die and a blue die. Let \m{E=\{\mbox{red die lands on 1}\}}, \m{F=\{\mbox{blue die lands on 1}\}}, \m{G=\{\mbox{red die lands mon 6}\}}. (a) Are \m{E} and \m{F} mutually exclusive? (b) How about \m{E} and \m{G}? (c) How about \m{F} and \m{G}?
...
<<hist10000,eval=F,echo=F,purl=FALSE>>=
