...
\item Normal random variables are the only ones we will need in this class.
...
mean(x)
@
\item The expectation of a random variable is sometimes called its mean. 
\end{myitemize}
...
\mydisplaymath{
 \var(\vect{y}) = \frac{1}{n} \sum_{i=1}^n (y_i-\bar{\vect{y}})^2
}
