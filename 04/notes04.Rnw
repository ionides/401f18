...
\begin{myitemize}
\item A continuous random variable \m{X} has a \myemph{probability density function} \m{f(x)} with
\mydisplaymath{
\prob(a < X < b) = \int_a^b f(x)\, dx
}
\item If \m{X} is \m{\normal(\mu,\sigma)}, i.e., \m{X} follows the normal distribution with mean \m{\mu} and sd \m{\sigma}, the probability density function is
\mydisplaymath{
...
\item \code{pnorm(25,mean=20,sd=5)} computes the shaded area:

<<normal_pdf,out.width="60mm",fig.width=3.5,fig.height=3,echo=F,purl=F>>=
...
Random draws from the normal distribution are obtained by \code{rnorm()} in R and we can use multiple \myemph{simulations} of the random  draws to learn about the distribution.

histogram(rnorm(n=100,mean=20,sd=5))
