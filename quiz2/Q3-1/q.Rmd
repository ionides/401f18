...
```{r, echo=F}
toxicity <- read.table("toxicity.txt",sep="")
```
...
```{r,child=paste("/sol_a.Rmd",sep=""),eval=SOL}
```
...
```{r,child=paste("/sol_b.Rmd",sep=""),eval=SOL}
```


```{r, echo = F}
plot(lm_toxicity$fitted.values,
...
(c) Based on the graph above and the corresponding summary statistics, is this model a good fit for the data? Do you have any concerns about using this model for this prediction.

```{r,child=paste("/sol_c.Rmd",sep=""),eval=SOL}
```
