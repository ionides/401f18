...
{\bf Pre-requisites}: This course builds on material in STATS 250. All course notes and labs for STATS 250 are at
...
\item USA data for 1933--2015 are in the file \url{life_expectancy.txt} 
...
\item The first lines of this file are:
\end{myitemize}
...
\myquestion.
Why should we prefer to use the command line form of R rather than a menu option, say in R Commander?

\answer{\vspace{10mm}}{(i) reproducibility; (ii) code sharing; (iii) scales to more complex statistical analysis; (iv) faster once learned.}
...
@
\ei
\myquestion. What do you think is the value of \code{v[3:5]}? 

\answer{15mm}{A vector with entries \code{4,1,5}}
\end{frame}
\begin{frame}[fragile]
