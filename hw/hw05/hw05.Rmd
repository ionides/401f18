...
An equivalent definition is that $\mat{A}$ is symmetric if $\mat{A}=\mat{A}^\transpose$. Reflection across the leading diagonal swaps rows and columns, which is exactly the same as transposing. Checking whether a matrix is equal to its tranpose can be a good way to see if a matrix is symmetric since we have already seen rules for transposing matrix products and inverses. 

**Q1**. Let $\mat{A}$ be a symmetric $p\times p$ matrix and let $\mat{B}$ be any $p\times q$ matrix. Let
\[
\mat{C} = \mat{B}^\transpose \mat{A}\mat{B}
\]
Show that $\mat{C}$ is equal to its own tranpose and hence show that $\mat{C}$ is a symmetric $q\times q$ matrix.
...
**Q2**. Let $\mat{A}$ be a symmetric $p\times p$ matrix with an inverse $\mat{A}^{-1}$. Is  $\mat{A}^{-1}$ symmetric? (i) Calculate an example using R to check this; (ii) Explain this result using properties of the matrix transpose and inverse.

--------------

**Q3**. Working with the bivariate normal distribution.
...
(e) Make a pairs plot of your three simulated variables. By looking at the pairs plot, guess the three correlations between the three possible choices of pairs of these variables. Then compute the exact correlations from `V` and see how close your guess was. 
...
**Q5**. Means and variances of sums and averages.

Let $\vec{X}=(X_1,X_2,X_3,X_4)$ be a vector random variable with mean ?? and variance/covariance matrix ??. Find the mean and variance of (i) sum X_i ; (ii) \bar X.

other simple exercises to reinforce notes.
