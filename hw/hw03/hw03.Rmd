...
(c) In R, use matrix multiplication to compute the vector $\vec{b}=(b_1,b_2)$ giving the least squares fit of this linear model. Use R notation matching the matrix form of the linear model,
\[ 
\vec{y} = \mat{X}\vec{b} + \vec{e}
\]
where $\vec{y}=(y_1,\dots,y_n)$, $\mat{X}=[x_{ij}]_{n\times 2}$ and $\vec{e}=(e_1,\dots,e_n)$.
...
(e) Use `lm()` in R to fit the same linear model. Check that you get the same fitted values. Report the first 5 fitted values obtained by both `lm()` and the matrix computation.
