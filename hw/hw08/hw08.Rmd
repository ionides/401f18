...
This homework continues the analysis of the newspaper circulation data from HW7. Refer back to HW7 for a description of the data and how to download it. We are going to 
```{r read_data}
...
(b) Find a standard error for this prediction and hence use a normal approximation to find a confidence interval for the expected logarithm of Sunday circulation.

(c) Figure out how to use the `predict()` function to get a confidence interval for the expected log Sunday circulation. If you look at `?predict` you will see that `predict()` is a _generic function_. This means that `predict(lm1,...)` will look at the class of `lm1` when working out what to do. Here, `lm1` has class `lm` and so the `predict()` should follow the rules for prediction using a fitted linear model. When `predict(lm1,...)` sees that `lm1` has class `lm`, it passes on its task to a function `predict.lm()`. Therefore, `?predict.lm` gives you the necessary syntax for calling `predict(lm1,...)`. In particular, you will have to look through the arguments described in `?predict.lm` to see which are relevant to this particular question.
...
**Q2**. Explain some things you would look for to check whether your confidence interval from Q1(b) is reasonable.

**Q3**. We now turn to the problem of finding an interval estimate for the circulation of the proposed new Sunday edition. 
...
(e) Transforming data, such as by taking the logarithm, is a useful technique to build a good statistical model but the transformation has to be undone (inverted) if you want to pull answers back to the scale of the original data. The inverse of the natural logarithm is the natural exponent. In math, 
\[
\log(e^x)=e^{\log(x)}=x
\] 
In R, 
...
exp(log(5))
```
Construct a prediction interval for Sunday circulation. This involves moving the prediction interval in (b) for log Sunday cirulation back to the original un-logged scale.

**Q4**. How compelling is the evidence that there is a real difference between newspapers which are tabloid with a serious competitor (`Competition==1`) and the other papers (`Competition==0`)? Address this question by specifying a suitable null hypothesis and making an appropriate hypothesis test. 
