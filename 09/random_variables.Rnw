...
\item
In other words, a random variable is a range of possible values with assigned probabilities.
\item 
...
\item Let's investigate some consequences of this idea.
\end{myitemize}
\end{frame}
...
\begin{myitemize}
\item Which of the following make sense, for data \m{y_1,\dots,y_n}

\vspace{1mm}

A. A simple model is \m{y_1,\dots,y_n\sim\normal(\mu,\sigma)}.

\vspace{3mm}

B. \m{\var(y_1)=\frac{1}{n-1}\sum_{i=1}^n (y_i-\bar y)^2} for \m{\bar y = \frac{1}{n}\sum_{i=1}^n y_i}.

\vspace{3mm}

C. In a linear model, \m{\E[\vec{b}] = \vec{\beta}}.

\vspace{3mm}

D. Because \m{\hat\beta_1 \pm 1.96 \sd{\hat\beta_1}} covers \m{\beta_1} with probability 0.95, we call \m{b_1 \pm 1.96 \SE{b_1}} a 95\% confidence interval.
