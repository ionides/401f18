...
\answer{\vspace{35mm}}{(i) well written scientific reports should be independ of a specific software choice; (ii) math is more precise and it requires you to check you understand your model; (iii) writing the model in math cross-checks against your R code.}
...
\answer{\vspace{50mm}}{Looking at the numbers, we see that adding interaction does not substantially decrease the estimated measurement error. We also see that these models are only a small amount better at prediction than a simple averagre. }
