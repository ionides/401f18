...
\answer{\vspace{50mm}}{The 95\% CI covers zero exactly when we fail to reject the null hypothesis that zero is the true value. Thus, the CI for the parameter with the smallest p-value covers the true value of zero only when all p-values are greater than 0.05. The chance that all p-values are greater than 0.05 under the null mypothesis is
$0.95^100=0.0059$.}
