...
\begin{myitemize}
\item Recall the mouse weight experiment. 24 mice are randomizes to one of two diets and are weighed after two weeks.
...
@
\myquestion. Write down the model R has fitted, in double subscript form, and interpret the parameters.
...
\myquestion. What is the model fitted by \code{lm(FGt~FGt1+Name,data=goals)}? Use double-subscript form.
