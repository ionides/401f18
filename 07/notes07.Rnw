...
\begin{frame}
\frametitle{Chapter~\CHAPTER. Factors and F-tests}

\begin{myitemize}
\item A \myemph{factor} is an explanatory variable with discrete levels.

\item Factors are also called \myemph{categorial variables}.
...
\item In linear models, factors can describe different classes of units. For example, in HW7, the binary factor \code{Competition} distinquishes certain types of newspaper.
...
\begin{myitemize}
\item Recall the mouse weight experiment. 12 mice are given each of two diets and are then weighed.
\item First, set up notation. Let \m{y_{ij}} be the weight of the \m{j}th mouse on treatent \m{i}, where \m{i=1,2} corresponds to the normal and high fat diet respectively and {j=1,\dots,12} enumerates the replicates for each treatment group.
...
\item Here, we have written the model in subscript form. We have a mean for each level of the treatment group factor.
...
in the usual linear model form,
\mydisplaymath{
...
\begin{myitemize}
\item 
Consider the following two models in subscript form, with \m{\epsilon_{ij}\sim\iid\;\normal(0,\sigma)} for \m{i=1,2} and  \m{j=1,\dots,12}.
...
\m{\mathrm{(M2).}\quad\quad Y_{ij}= \mu + \alpha_i + \epsilon_{ij} \quad \mbox{with $\mu_1=0$}}
